...
const locusGroupLabelsBN = {
    "Multiple Alleles of bl-locus": "bl-লোকাসের অ্যালিলসমূহ",
    "Allelic Compounds of bl-locus": "bl-লোকাসের অ্যালিলিক কম্পাউন্ড",
    "Multiple Alleles of a-locus": "a-লোকাসের অ্যালিলসমূহ",
    "Allelic Compounds of a-locus": "a-লোকাসের অ্যালিলিক কম্পাউন্ড",
    "Multiple Alleles of dil-locus": "dil-লোকাসের অ্যালিলসমূহ",
    "Multiple Alleles of ino-locus": "ino-লোকাসের অ্যালিলসমূহ",
...
window.applyCompoundSplitFix = function(sex, notationStr) {
    const prefix = sex === 'male' ? 'sire' : 'dam';
    const inputEl = document.getElementById(`${prefix}-search-input`);
    inputEl.value = notationStr;
    handleSearchInput(sex);
};
...
// Builds the "D green" / "DD green" style display prefix, mirroring the same
// dark-factor-symbol + base-color convention used by formatModWord() in the
// results engine. Used both for the human-readable suggestion links and, via
// applyCompoundSplitFix, as the literal text written back into the search box
// on click — the parser accepts "D"/"DD"/"SF"/"DF" and "/", "//" directly.
function formatVisPrefix(foundTraits, sex) {
    let modifiers = foundTraits
        .map(t => ({ trait: t, m: mutationDB.find(x => x.id === t.id) }))
        .filter(o => o.m && o.m.cat > 1)
        .sort((a, b) => nameRank(a.m.cat) - nameRank(b.m.cat));

    let words = modifiers.map(o => {
        let m = o.m, trait = o.trait;
        if (m.id === "dark_factor") return trait.val === 1 ? "D" : "DD";
        let pName = m.result_label || m.name;
        let zyg = null;
        if (m.type.includes("SL") && sex === "female") {
            if (m.type === "SLID" || m.type === "SLD") zyg = "SF";
        } else if (m.type === "AD" || m.type === "AID" || m.type === "SLID" || m.type === "SLD") {
            zyg = trait.val === 1 ? "SF" : "DF";
        }
        return zyg ? `${zyg} ${pName}` : pName;
    });

    let baseColorNames = foundTraits
        .map(t => mutationDB.find(x => x.id === t.id))
        .filter(m => m && m.cat === 1)
        .map(m => m.result_label || m.name);

    words.push(baseColorNames.length > 0 ? baseColorNames.join(" ") : "green");
...
function processSearchQuery(query, species, sex) {
    ...
    if (compoundMut) {
        ...
        let arrNames = arrIds.map(id => {
            ...
            return dbm ? (dbm.result_label || dbm.name) : id;
        });
        
        let visPrefix = formatVisPrefix(parsedVis.foundTraits, sex);
        
        let opts = [];
        if (arrIds.length >= 3) {
            let a0 = arrNames[0], a1 = arrNames[1], a2 = arrNames[2];
            let notation1 = `${visPrefix}/${a0}//${a1}`;
            let notation2 = `${visPrefix}/${a0}//${a2}`;
            let notation3 = `${visPrefix}/${a1}//${a2}`;
            opts.push(`1. <a href="javascript:void(0)" onclick="applyCompoundSplitFix('${sex}', '${notation1.replace(/'/g, "\\'")}')" style="color:#007bff;text-decoration:underline;">${notation1}</a>`);
            opts.push(`2. <a href="javascript:void(0)" onclick="applyCompoundSplitFix('${sex}', '${notation2.replace(/'/g, "\\'")}')" style="color:#007bff;text-decoration:underline;">${notation2}</a>`);
            opts.push(`3. <a href="javascript:void(0)" onclick="applyCompoundSplitFix('${sex}', '${notation3.replace(/'/g, "\\'")}')" style="color:#007bff;text-decoration:underline;">${notation3}</a>`);
            ...
            if (existingIdx !== -1) {
                ...
                if (t1.id === t.id) {
                    ...
                    if (compound) {
                        ...
                        let a0 = (m1.result_label || m1.name);
                        let a1 = (m2.result_label || m2.name);
                        
                        let opts = [];
                        let notation1 = `${visPrefix}/${a0}//${a1}`;
                        let notation2 = `${visPrefix}/${a1}//${a0}`;
                        opts.push(`1. <a href="javascript:void(0)" onclick="applyCompoundSplitFix('${sex}', '${notation1.replace(/'/g, "\\'")}')" style="color:#007bff;text-decoration:underline;">${notation1}</a>`);
                        opts.push(`2. <a href="javascript:void(0)" onclick="applyCompoundSplitFix('${sex}', '${notation2.replace(/'/g, "\\'")}')" style="color:#007bff;text-decoration:underline;">${notation2}</a>`);
